export default function Timeline({ events }) {
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        <h2 className="text-2xl font-semibold mb-6">Verified Timeline</h2>
        <div className="relative">
          <div className="absolute left-8 top-0 bottom-0 w-0.5 bg-blue-200"></div>
          
          {events.map((event, i) => (
            <div key={i} className="relative pl-16 pb-8 last:pb-0">
              <div className="absolute left-6 top-1 w-5 h-5 rounded-full bg-blue-500 border-4 border-white shadow"></div>
              
              <div className="bg-gray-50 rounded-lg p-4 shadow-sm">
                <div className="flex items-start justify-between mb-2">
                  <p className="font-semibold text-gray-900">{formatDate(event.datetime)}</p>
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getConfidenceBadge(event.confidence)}`}>
